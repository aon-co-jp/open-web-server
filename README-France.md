...
6. **Diffusion de fichiers statiques + PHP** (`static_files`/`php_server`/`web_vhost`, 2026-07-20) — premier pas vers un moteur de diffusion hybride Apache+Nginx. Chaque nom d'hôte est associé à un docroot ; les chemins reconnus comme des ressources statiques par leur extension sont servis directement depuis le disque (avec protection anti path-traversal), le reste étant transmis à un sous-processus `php -S` démarré à la demande. Vérifié en servant réellement `audiocafe.tokyo` (un site PHP existant).
