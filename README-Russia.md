...
5. **Целевая архитектура: четырёхкратная избыточность транспорта и записи в БД** (пересмотрено 2026-07-11) — конечная цель: транспорт через TCP-IP + UDP-IP + QUIC/MPQUIC + MPTCP/SCTP, запись в PostgreSQL (свойства транзакций ACID: атомарность, согласованность, изолированность, долговечность) + aruaru-db + синхронную мультирегиональную репликацию + независимый журнал аудита. На данный момент реализованы только TCP-IP и UDP-IP (без повторной отправки), остальное ещё не начато (подробности: [README-Japan.md](README-Japan.md#6-目標アーキテクチャ-通信層dbの四重化), [CLAUDE.md](CLAUDE.md)).
