...
> **2026-07-24追記**: nginxをopen-web-server自身のTLS終端(ACME自動取得)+
> `web_vhost`/`tenant_router`へ完全移行する作業を実施中(本番VPS)。
> 全ドメインの実Let's Encrypt証明書取得まで完了、最終カットオーバー
...
**既知の残課題(2026-07-30続き、次回対応予定)**: `KeyGuardian`が発行する
キーの`roles`フィールドは保持されるのみで、実際にアクセス範囲を制限する
検査が行われていない——有効なキーであれば`owner`/`roles`を問わず
`/admin/*`全体(今回保護した`/internal/db/state/...`含む)へ同一の権限を
持ってしまう。他人への「なりすまし」の実害は無いが、role単位の最小権限
分離が未実装のため次回実装対象として記録する(詳細は`CLAUDE.md`
HANDOFF参照)。
