...
10. **無料DDNS(DuckDNS)自動ドメイン取得〜自動更新**(`ddns` feature、2026-07-23) — 固定IPを持たない環境向けに、無料で・有効期限切れの心配無く使えるサブドメイン(DuckDNS、No-IPと異なり30日ごとの手動確認が不要)を`OPEN_WEB_SERVER_DUCKDNS_DOMAIN`/`OPEN_WEB_SERVER_DUCKDNS_TOKEN`の2環境変数だけで自動取得・自動更新する(`free_domain.rs`)。管理API`POST /admin/ddns/setup-free-domain`で即時疎通確認、`GET /admin/sftp/connection-info`は生IPよりDuckDNSホスト名を優先して返すため「一度設定すれば変わらない」SFTP接続コマンドが得られる。**正直な開示**: DuckDNSアカウント自体の取得(トークン発行)はユーザー自身のOAuthログインが必要で自動化しない。DuckDNS実サービスへの実接続は本開発環境からは未検証(モックHTTPサーバーでのロジック検証のみ)。`open-easy-web`側に対応するウィザードUIも追加済み(詳細は[PORTING.md §4.9](PORTING.md#49-無料ddnsduckdns自動ドメイン取得自動更新2026-07-23新設ddns-feature配下)、[CLAUDE.md](CLAUDE.md)の同日HANDOFF参照)。
