...
6. **目標アーキテクチャ: 通信層・DB書き込みの四重化**(2026-07-13更新) — 課金/金融/証券/クレジットカードデータをネットワーク上で失わないため、通信層は TCP-IP・UDP-IP・QUIC/MPQUIC・MPTCP/SCTP の4方式、DB書き込みは PostgreSQL(ACID＝原子性・一貫性・独立性・永続性を保証するトランザクション特性)・aruaru-db・マルチリージョン同期レプリケーション・独立監査ログの4系統を目標とする。**2026-07-12実装**: ③QUIC(`open-web-server-wire::quic_channel`, `quinn`ベース、実TLS1.3ハンドシェイク+双方向ストリームを実UDPソケットで検証済み)、PostgreSQL WAL(`open-web-server-ledger::PostgresWal`, `sqlx`ベース、実BEGIN/COMMITトランザクション。ライブPostgreSQL接続はこのサンドボックス環境に無く未検証、SQL構築ロジックの単体テストのみ)を追加。**2026-07-13実装**: ④MPTCP/SCTPを調査した結果、Windows開発環境ではカーネルMPTCP/SCTPの実ソケット検証が不可能と判明(正直なブロッカー)。代わりに同じ目的(物理経路マルチホーミング)をユーザー空間で実現する`aggligator`クレートを用いた代替(`open-web-server-wire::mptcp_channel`)を実装し、実ループバックTCPソケットでのラウンドトリップ結合テストで検証済み(本物のカーネルMPTCP/SCTPではないことを明記)。aruaru-db側では「次回新規開発予定」だったaruaru-dbコミット×open-raid-zスナップショット連携の第一段実装(`aruaru-dist::snapshot_pairing`)も完了。四重DB書き込みの残り3系統(②aruaru-db・③マルチリージョン同期レプリケーション・④独立監査ログ)は引き続き未着手(詳細は [README-Japan.md](README-Japan.md#6-目標アーキテクチャ-通信層dbの四重化) と [CLAUDE.md](CLAUDE.md#拡張要件2026-07-11ユーザー指示目標アーキテクチャ実装は段階的に))。
