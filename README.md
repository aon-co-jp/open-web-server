...
13. **省メモリ/省電力モードの実行時切替(途中からでも変更可)**(2026-07-26) — Android版はもとより`switchProfileAndRestart()`(再起動が必要)しか無かったが、稼働中の`MainActivity`から`WakeLock`取得/解放・ヘルスチェックポーリング間隔・ログ保持サイズを**再起動無しで**即座に切り替えられるようにした。デスクトップ(Windows/Linux)版にも、Android版`PowerProfile.kt`と同じ4プロファイル名・ラベル(`memory_saver`/`power_save`/`normal`/`always_on`)を持つ`power_profile.rs`を新設し、`GET`/`POST /admin/power-profile`で再起動無しに切替可能(実際の挙動差は`ddns`/`free_domain`のバックグラウンドポーリング間隔調整のみ、正直に狭いスコープ)。実HTTP経由の統合テストで「同一プロセスのまま値が即座に切り替わる」ことを検証済み(詳細は[CLAUDE.md](CLAUDE.md)の2026-07-26 HANDOFF参照)。
