//! open-web-server: ライブラリ本体(`run()`が実質的なエントリポイント)。
//!
...
//!
//! 2026-08-05: 通常のバイナリ(`src/main.rs`、`#[tokio::main]`で`run()`を
//! 呼ぶだけの薄いラッパー)に加え、iOS向け`open-web-server-ios-bridge`
//! クレート(`crates/open-web-server-ios-bridge`)がこの`run()`をFFI経由で
//! 呼び出せるよう、ロジック本体をバイナリcrateからライブラリcrateへ移設
//! した(内容は従来の`main.rs`そのまま、`main()`を`pub async fn run()`へ
//! 改名し`#[tokio::main]`属性を外しただけ——呼び出し側がランタイムを
//! 用意する設計に変更)。
...
/// サーバー本体を起動する(呼び出し元が`tokio`ランタイムを用意していること
/// が前提。通常のバイナリ〈`src/main.rs`〉は`#[tokio::main]`から、
/// iOSブリッジ〈`open-web-server-ios-bridge`〉は独自に構築した
/// `tokio::runtime::Runtime`上からこれを呼ぶ)。
