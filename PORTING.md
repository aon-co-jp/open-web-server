...
> 対象バージョン: workspace 0.1.0(4クレート / 13テスト、2026-07-11実測)。
...
3層防御通信・冪等WAL + 3ホップコミット・UDP-IP冗長経路により、
...
│   ├── open-web-server-wire/     ← 3層防御通信 + UDP-IP冗長経路(udp_channel)
...
(13テストが通れば移設成功)。以下はライブラリとして個別に使う場合。
...
```rust
use open_web_server_wire::{PayloadCipher, MutualAuthConfig, TlsServerConfig};
...
cargo test --workspace     # 13テストが全部通ればOK(gateway 4 / ledger 3 / wire 6)
