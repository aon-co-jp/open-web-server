...
### 2026-08-05(続き) 自社ドメイン(custom_dns.rs)の管理API配線+
DDNS/カスタムドメイン登録時のHTTPS自動取得オプションを追加(ユーザー
指摘「固定IPなしで通常ドメインを自動更新する機能は未配線」への対応、
続けてユーザー質問「DuckDNSも通常ドメインもhttpsの最後がsに簡単にでき
る仕様は大丈夫?」への対応)

**前回セッションで発見した実態**: `custom_dns.rs`の`ValueDomainProvider`/
`ConohaDnsProvider`(自社ドメイン`aon.co.jp`/`runo.tokyo`等配下への無料
サブドメイン発行)は実装済みだったが、どの管理APIエンドポイントからも
バックグラウンドループからも呼ばれておらず、単体テスト以外では
一切使われない「配線されていないコード」だった(2026-07-24時点の
HANDOFFにも「管理APIハンドラへの配線は次回課題」と正直に記録済み)。

**今回の実装(配線)**:
1. **`CustomDomainRegistry`**(`custom_dns.rs`新設): 「FQDN→(ベース
   ドメイン, サブドメイン)」のメモリ帳簿(`free_domain::DomainRegistry`
   と同じ`RwLock<HashMap<..>>`パターン)。**意図的にネットワーク呼び
   出しを持たない**設計とし、実際のDNSプロバイダAPI呼び出しは呼び出し元
   (ハンドラ・自動更新ループ)が`build_provider()`経由で行う——これに
   より資格情報・実ネットワーク無しでメモリ帳簿ロジックの単体テストが
   書ける(既存の`free_domain::DomainRegistry`と同じ分離思想)。
2. **`build_provider(base_domain)`**: `aon.co.jp`→`ValueDomainProvider`、
   `runo.tokyo`/`nasa.tokyo`/`icpo.tokyo`→`ConohaDnsProvider`を環境変数
   から構築する振り分け関数(feature無効時でもコンパイルでき、実際の
   ネットワーク呼び出しは各`DnsProvider`実装側で従来通りfeature分岐)。
3. **5分間隔の自動更新ループ**(`custom_dns::net::run_loop`、
   `custom_domain` feature配下): `free_domain.rs`のDuckDNS自動更新
   ループと全く同じパターン(`api.ipify.org`でグローバルIP変化を検知
   →登録済み全ドメインを`update_ip`)。
4. **新規`handlers/custom_dns.rs`**: `POST /admin/custom-dns/setup`
   (`{base_domain, subdomain}`)・`GET /admin/custom-dns/domains`・
   `DELETE /admin/custom-dns/domains/:fqdn`(既存の`x-admin-token`/
   `KeyGuardian`認証を再利用)。`lib.rs`の`dispatch()`・`AppState`
   (`custom_domains`フィールド)・`run()`(バックグラウンドループ起動)
   へ配線。
5. **HTTPS自動取得(ユーザー追加質問への対応)**: 新規`crate::acme::
   try_auto_https(state, host, contact_email)`(`acme` feature)——
   既存の手動エンドポイント`POST /admin/tenants/:host/tls/acme`と
   全く同じ`obtain_certificate_http01`+`TenantCertResolver::
   upsert_pem`の経路を再利用する薄いヘルパー。`SetupFreeDomainRequest`
   (DuckDNS)・`SetupCustomDnsRequest`(自社ドメイン)の両方に任意
   `contact_email`フィールドを追加し、指定されていればドメイン登録の
   疎通確認が成功した直後に**1回だけ**Let's Encrypt HTTP-01証明書
   取得を試みる(レスポンスの`https_ready`/`https_message`で結果を
   正直に報告)。**意図的に自動リトライは実装していない**——DNS伝播が
   間に合わなければ、`https_message`が数分後に手動エンドポイントを
   再度呼ぶよう案内する設計(無条件の長時間リトライで登録リクエスト
   自体のレスポンスを塞がないため)。
6. **検証(型チェックのみで完了と報告しない、既存運用ルール徹底)**:
   `cargo build --workspace`/`cargo build -p open-web-server-gateway`
   (default/`--features ddns,acme`/`--features custom_domain,acme`の
   3通り)いずれもエラー0件。`cargo test -p open-web-server-gateway
   --lib --features ddns,acme,custom_domain`**193件全green**(新規:
   `custom_dns::`レジストリ単体テスト6件+実HTTP経由の統合テスト
   `custom_dns_domains_list_and_delete_work_over_real_http`——認証無し
   401・一覧2件+残り枠18件・非対応ベースドメインで400・削除後1件+
   404、を実TCP接続で確認)。
7. **正直な開示・未検証事項**: (1) 実Value-Domain/ConoHa DNS APIへの
   実接続は今回も未実施(実資格情報がこの環境に無い、既存の制約と
   同じ)。(2) `try_auto_https`の実際のDNS伝播待ち→ACME成功という
   フルE2Eも、実DNSプロバイダ接続が必要なため未検証。(3) Value-Domainの
   ゾーン全体送信方式(既存レコードとのマージ未実装)・ConoHa DNSの
   record_id事前取得(更新/削除の正確な実装に必要)は前回セッションから
   持ち越しの既知ギャップのまま(今回のスコープは「配線」であり、
   プロバイダ実装自体の精度向上は対象外)。

- 次にすべきこと: (1) 実Value-Domain/ConoHa資格情報が用意でき次第の
  実API接続E2E検証、(2) Value-Domainのゾーンマージ・ConoHa DNSの
  record_id解決の精度向上、(3) Android/iOS版UIへの「自社ドメイン」
  登録導線の追加(現状はDuckDNSのみAndroid版`DdnsSetupActivity`に
  UIがある、`custom_dns`側はサーバーAPIのみ)。
