...
`#[ignore]`統合テストで検証可能性を確保)。**④独立監査ログは今回パスで
実装完了**(`open-web-server-ledger::audit_log::FileAuditLog`、上記
進捗ノート参照)。②aruaru-db・③マルチリージョン同期レプリケーションは
未着手。(1)は未着手(調査済み・下記参照)。
...
- `cargo check --workspace` / `cargo test --workspace` は成功する(4クレート構成、
  2026-07-13時点(監査ログ実装後)で全31テストがgreen(+1件は要ライブ
  PostgreSQLの`#[ignore]`): gateway 4件 / ledger 13件(postgres_wal SQL
  構築ロジック4件・audit_logの単体3件+`Ledger::commit`経由の統合1件を
  追加) / wire 14件(quic_channel結合テスト2件・mptcp_channel結合テスト
  1件を追加)。
