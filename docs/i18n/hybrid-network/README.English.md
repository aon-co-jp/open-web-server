...
**Update (v0.9):** the Poem/Tauri reproduction mission is now stated explicitly: both are developed from scratch and reproduced with full compatibility, including in-browser execution. Also fixed stale one-directional lead/mirror wording and left-aligned doc-link lists in open-runo/poem-cosmo-tauri READMEs. Note: no live web-search tool was available this session for the §0.8 research directive.
