...
**Direttiva per la prossima sessione (v0.8):** continuare a fondere l'ibrido ZFS+ACID tra i repository, e ricercare (via Google/GitHub in giapponese E inglese, non solo a memoria) poi implementare lo stack di trasporto a 4 livelli/4 ridondanze (TCP-IP/UDP-IP/QUIC-MPQUIC/MPTCP-SCTP) -- come UNA pipeline fusa a zero perdite per dati di giochi/finanza/titoli/carte di credito, non funzionalità separate. Vedi §0.8.

**Aggiornamento (v0.9):** la missione di riproduzione di Poem/Tauri è ora dichiarata esplicitamente: entrambi vengono sviluppati da zero e riprodotti con piena compatibilità, inclusa l'esecuzione nel browser. Corrette anche le formulazioni obsolete su una direzione unica lead/mirror e allineate a sinistra le liste di link alla documentazione nei README di open-runo/poem-cosmo-tauri. Nota: in questa sessione non era disponibile alcuno strumento di ricerca web live per la direttiva di ricerca del §0.8.
