...
**下次会话指令(v0.8):** 继续在各仓库中融合 ZFS+ACID 混合技术,并通过日语和英语的 Google/GitHub 搜索(而非仅凭记忆)调研后实现四层四重传输栈(TCP-IP/UDP-IP/QUIC-MPQUIC/MPTCP-SCTP)——作为游戏道具/金融/证券/信用卡数据零丢失的统一管道,而非各自独立的功能。详见 §0.8。

**更新(v0.9):** 明确表述“Poem 与包含浏览器内执行能力的 Tauri 二者均从零开发并实现完全兼容复刻”。同时修正了 open-runo/poem-cosmo-tauri README 中过时的单向领先/镜像说法,并将文档链接列表左对齐。注:本次会话没有可用于 §0.8 调研指令的实时网络搜索工具。
