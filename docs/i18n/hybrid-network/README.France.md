...
**Mise à jour (v0.9) :** la mission de reproduction de Poem/Tauri est désormais énoncée explicitement : les deux sont développés à partir de zéro et reproduits avec une compatibilité totale, y compris l'exécution dans le navigateur. Correction également des mentions obsolètes sur la direction unique tête/miroir et alignement à gauche des listes de liens vers la documentation dans les README d'open-runo/poem-cosmo-tauri. Remarque : aucun outil de recherche web en direct n'était disponible dans cette session pour la directive de recherche du §0.8.
