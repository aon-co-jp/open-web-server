...
open-runo→aruaru-db) は同一方針の4層で保護する。
...
3. **第3層 (payload_crypto.rs)**: ChaCha20-Poly1305 (AEAD) によるアプリケーション層
   ...
   暗号化されたまま流れる
4. **第4層 (replay_guard.rs, 2026-07-11 追加)**: seq(単調増加シーケンス番号) +
   timestamp によるリプレイ(再送)対策。第3層のAEADは改ざん検知はするが
   「正規の暗号文をそのまま再送する」攻撃は防がないため、seq/timestampを
   AEADのAADとして暗号文に暗号学的に紐付け、受信側で既知seqの再受信・
   許容時刻窓(30秒)外のtimestampを拒否する。課金/決済確定のような
   非冪等操作の二重適用を防ぐ最終防衛ライン。`SecureChannel` が第3層+
   第4層を1本にまとめた送受信ヘルパーとして提供される。
...
具体的な実装**。上記「4層防御通信」はTLS/相互認証/ペイロード暗号化/
リプレイ対策という**セキュリティレイヤー**のスタックであり、単一のTCPコネクション上に積む
